...
for subj = 1:3
    ...
    switch subj
        case 1
            testset = [testset(:,1:54),testset(:,56:end)];
        case 2
            testset=[testset(:,1:20),testset(:,22:37),testset(:,39:end)];
        case 3
            testset = testset;
    end
    ...
    predictions_ridge = Prediction_Ridge(weight,train_limits, testset,sr,duration,...
        windowSize,displ,chosenFeatures,history,subj);
    
%     load(strcat('chosenfeatures_logreg_sub',num2str(subj),'.mat'));
%     
%     predictions_logreg = Prediction_LogReg(weights_logreg{subj},train_limits, ...
%             testset,sr,duration,windowSize,displ,chosenFeatures,history);
%     
%     load(strcat('chosenfeatures_ensemble_sub',num2str(subj),'.mat'));
%     
%     prediction_ensemble = Prediction_Ensemble(models_ensemble{subj},train_limits, ...
%             testset,sr,duration,windowSize,displ,chosenFeatures,history,subj);
%     
%     pred_classifier = zeros(size(predictions_logreg));
%     for i=1:5    
%         pred_classifier(:,i) = mean([(predictions_logreg(:,i)),(prediction_ensemble(:,i))],2);
%     end    
        
    predictions = predictions_ridge;%.*pred_classifier;
