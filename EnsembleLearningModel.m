function [ models,chosenFeatures ] = EnsembleLearningModel( train_ecog_data,train_labels,samplingRate,windowSize,displ,subject,history )
...
save(strcat('features_ensemble_sub',num2str(subject),'.mat'),'featureMat');
...
save(strcat('ranks_ensemble_sub',num2str(subject),'.mat'),'ranks');

chosenFeatures = unique(features);

save(strcat('chosenfeatures_ensemble_sub',num2str(subject),'.mat'),'chosenFeatures');
...
%featureMat = FeatureHistoryGeneration( featureMat,history );
    ...
    for i=1:5
         m= fitensemble(featureMat,trainlabels_decimated(:,i),'LogitBoost',1000,'tree');
         models{i} = compact(m);
    end
