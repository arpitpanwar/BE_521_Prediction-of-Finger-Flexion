...
for i=0:61
    ...
    num1 = FilterDesign(5,15);
    featureMat(:,i*6+2) = MovingWinFeats(filtfilt(num1.coefficients{1},1,curr),sr,0.1,0.05,@(x)mean(x));
    
    num1 = FilterDesign(20,25);
    featureMat(:,i*6+3) = MovingWinFeats(filtfilt(num1.coefficients{1},1,curr),sr,0.1,0.05,@(x)mean(x));

    num1 = FilterDesign(75,115);
    featureMat(:,i*6+4) = MovingWinFeats(filtfilt(num1.coefficients{1},1,curr),sr,0.1,0.05,@(x)mean(x));

    num1 = FilterDesign(125,160);
    featureMat(:,i*6+5) = MovingWinFeats(filtfilt(num1.coefficients{1},1,curr),sr,0.1,0.05,@(x)mean(x));
    
    num1 = FilterDesign(160,175);
    featureMat(:,i*6+6) = MovingWinFeats(filtfilt(num1.coefficients{1},1,curr),sr,0.1,0.05,@(x)mean(x));
...
weight_mat = featureMat'*featureMat\(featureMat'*featureMat); 
