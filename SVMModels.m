function [ models,chosenFeatures ] = SVMModels( train_ecog_data,train_labels,...
    ...
    if ~savefileExists(featureFile)
        ...
        save(strcat('featuresMovement_',num2str(subject),'_v',num2str(save_version),'_2.mat'),'featureMat');
    else
        load(featureFile);
    end
...
%   train_labels = train3_labels;
    ...
    for i = 1:2
        if ~savefileExists(ranksFile)
            ...
            save(strcat('reductionRanksMovement_',num2str(subject),'_labels_,',size(train_labels,2),'v',num2str(save_version),'_2.mat'),'ranks');  
        else
            load(ranksFile);
    ...
    strcat('reductionRanksMovement_',num2str(subject),'_labels_,',size(train_labels,2),'v',num2str(save_version),'_2.mat');
