function [ models] = SVMModels( train_ecog_data,train_labels,samplingRate,windowSize,displ,subject )
...
%     end
       trainlabels_decimated = decimate(train_labels,displ*10^3);
...
%     weight_mat = zeros([size(featureMat,2),5]);
        ...
        trainlabels_decimated(trainlabels_decimated >=  0) = 1;
        trainlabels_decimated(trainlabels_decimated <= 0) = 0;

        c = zeros([100,1]);
        B = lasso(featureMat,trainlabels_decimated);
        for j=1:100
            c(j) = corr(featureMat*B(:,j),trainlabels_decimated);
        
            weight_mat = B(:,find(max(c)));
        end          
    %Generating weight matrix
    models = cell(size(trainlabels_decimated,2),1);
    disp 'Generating models';
    for i=1:size(models,1)
       models{i} = fitcsvm(featureMat*weight_mat,trainlabels_decimated,'Standardize',true);
